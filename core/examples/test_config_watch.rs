...
fn main() {
    ...
    let mgr_arc =
        config_loader::get_config_manager().expect("Config manager should be initialized");
    ...
    let update_config_file = |path: &std::path::PathBuf, key: &str, value: i64| {
        ...
        let new_content =
            serde_json::to_string_pretty(&json_val).expect("Failed to serialize config");
