...
use crate::rdbms_reader_util::util::dbpool::{detect_db_kind, get_db_pool};
use crate::rdbms_reader_util::util::dbpool::{DbKind, DbPool};
use data_trans_common::app_config::config_loader::get_config_manager;
use data_trans_common::job_config::JobConfig;
use data_trans_common::resp::BaseDbQuery;
...
pub fn build_query_sql(table: &str, query: Option<&str>, limit: usize, offset: usize) -> String {
    // 如果 query 存在，则在其基础上添加 LIMIT 和 OFFSET；否则构建一个基本的 SELECT 语句。
