...
pub trait ReaderJob: Send + Sync {
    ...
    /// 执行单个分片任务
    async fn execute_task(&self, task: ReadTask, tx: mpsc::Sender<PipelineMessage>) -> Result<usize>;
    /// 返回描述信息
...
pub trait ReaderTask: Send + Sync {
    /// 读取数据并发送到 Channel
    async fn read_data(&self, task: &ReadTask, tx: &mpsc::Sender<PipelineMessage>) -> Result<usize>;
}
